...
#only store 2^16 sample. Extra 4 bit to specify which ADC is running
non_ideal_output_in_bit = np.zeros((2**simulation_bit//64*4, n_bits_*4 + 4), dtype=np.float32)
ideal_output_in_double  = np.zeros(2**simulation_bit//64*4, dtype=np.float32)

adc_step = v_ref_ / (2**n_bits_)



temp = np.zeros(n_bits_*4 + 4, dtype=np.float32)
non_ideal_output_bit_seperate = np.zeros((2**simulation_bit//64*4, n_bits_), dtype=np.float32)

for i in range(2** simulation_bit//64):
    temp[0:n_bits_] = sar_non_ideal_1.digitize(input_signals_1[i*4])
    temp[n_bits_*4:n_bits_*4+4] = [1,0,0,0]
    non_ideal_output_in_bit[i*4]   = temp
    non_ideal_output_bit_seperate[i*4] = temp[0:n_bits_]

    temp[n_bits_:2*n_bits_] = sar_non_ideal_2.digitize(input_signals_2[i*4])
    temp[n_bits_*4:n_bits_*4+4] = [0,1,0,0]
    non_ideal_output_in_bit[i*4+1] = temp
    ...
    temp[n_bits_*4:n_bits_*4+4] = [0,0,1,0]
    non_ideal_output_in_bit[i*4+2] = temp
    ...
    temp[n_bits_*4:n_bits_*4+4] = [0,0,0,1]
    non_ideal_output_in_bit[i*4+3] = temp
    non_ideal_output_bit_seperate[i*4+3] = temp[3*n_bits_:4*n_bits_]

    ideal_output_in_double[i*4] = np.floor(input_signals_1[i*4] / adc_step)*adc_step
    
    ideal_output_in_double[i*4+1] = np.floor(input_signals_1[i*4+1] / adc_step)*adc_step

    ideal_output_in_double[i*4+2] = np.floor(input_signals_1[i*4+2] / adc_step)*adc_step

    ideal_output_in_double[i*4+3] = np.floor(input_signals_1[i*4+3] / adc_step)*adc_step
...
plt.savefig("Image/TI_ADC_CALIBRATION/nonideal_output_signal.png", dpi=300, bbox_inches="tight")
